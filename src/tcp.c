...
const char *SERVER_ADDR = "192.168.1.101";
...
int tcp_read(int isocket, void * pvbuff, unsigned int * pisize)
{
	...
	//Show(0, "tcp_read: error=%d", error);
	if (error > 0)
	{
		if (*pisize > 0)
		{
			error =  recv(isocket, (char*)pvbuff, *pisize, 0);
			//Show(0, "recv: error=%d", error);
			if (error > 0)
			{
			...
			{
				//Show(0, "555: error=%d, pisize=%d", error, *pisize);
				//find: error == 0 and *pisize > 0
				retval = -1;
				return retval;
			}
		}
...
int TCP_Get(unsigned char *recvData, unsigned int *pRecvLen, const unsigned char *sendData, unsigned int sendLen)
{
	...
	*pRecvLen = 0;
	...
	ret = tcp_write(fd, sendData, sendLen);
	if (ret != 0) {
		Show(0, "tcp_write failed!");
		return ret;
	}

	ret = tcp_read(fd, szRecvBuf, &recvBufLen);
...
int TCP_WaitGet(unsigned char *recvData, unsigned int *pRecvLen, const unsigned char *sendData, unsigned int sendLen)
{
	...
	*pRecvLen = 0;
	...
	//add: 5 sencond x|ping. example: 20160324202156|ping
	now = GetTime();
	while (*pRecvLen <= sizeof(szPing))
	{
		...
		now = GetTime();
		memset(szPing, 0, sizeof(szPing));
		GetPingString(szPing);
		//Show(-1, "tcp_write ping: %s", szPing);
		ret = tcp_write(fd, szPing, strlen(szPing));
		if (ret != 0) {
			Show(0, "tcp_write failed: data=%s, ret=%d", szPing, ret);
			break;
		}
		if (*pRecvLen > 0)
		{
			memset(recvData, 0, *pRecvLen);
			*pRecvLen = 0;
			pos = NULL;
			pos = recvData;
		}
		...
		if (ret != 0) {
			//Show(0, "tcp_read failed: ret=%d, pRecvLen=%d", ret, *pRecvLen);
			break;
		}
		//Show(-1, "recv: %s, pRecvLen=%d", recvData, *pRecvLen);
	}
	tcp_close(fd);
	return ret;
}
